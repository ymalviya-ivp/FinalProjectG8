...
const TradeBlotter = () => {
    ...
    const fetchFilterOptions = async () => {
        try {
            const response = await axios.get('https://localhost:7021/api/trades/filters');
            if (response.status === 200) {
                // Ensure arrays are populated, even if API returns lowercase keys
                setFilterOptions({
                    securityIds: response.data.securityIds || response.data.SecurityIds || [],
                    tradeDates: response.data.tradeDates || response.data.TradeDates || []
                });
            }
        } catch (err) {
            console.error("Failed to load filter options", err);
    ...
    return (
        <div className="panel">
            <form onSubmit={handleSearch} className="toolbar">
                ...
                <div className="filter-group">
                    <label>Trade Date</label>
                    <select className="enterprise-select" value={tradeDate} onChange={(e) => setTradeDate(e.target.value)}>
                        <option value="">-- All Dates --</option>
                        {filterOptions.tradeDates.map(date => (
                            <option key={date} value={date}>{date}</option>
                        ))}
